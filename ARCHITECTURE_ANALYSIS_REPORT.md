...
## 1. Executive Summary
Mr. Clipper (v1.0.1) is a sophisticated, modular video processing system designed for intelligent highlight extraction and clipping. It follows a "Core Engine + CLI + Skill Wrapper" architectural pattern, prioritizing maintainability, typed configuration, and robust integration with external multimedia tools like `ffmpeg` and `yt-dlp`.
...
2.  **CLI Layer (`mrclipper/mrclipper/cli.py`):** A `Typer`-powered interface that orchestrates the core components into user-facing commands (`clip`, `auto_highlight`, `search`).
...
### 2.2 Data Flow
1.  **Initialization:** The CLI loads configuration using Pydantic models, merging defaults, global settings (`~/.config/mrclipper/config.toml`), and optional job-specific overrides.
...
3.  **Processing:**
    ...
    *   **Analysis (Mode B):** For highlights, `ffmpeg` filters (`scene`, `astats`) are used to detect visual transitions and audio peaks.
...
*   **Detection:** Parallel analysis of scene changes and audio peaks.
